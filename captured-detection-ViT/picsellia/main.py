...
train_test_valid_dataset["train"] = train_test_valid_dataset["train"].with_transform(
    transform_aug_ann
)
...
path_output, path_anno = save_annotation_file_images(
    dataset=train_test_valid_dataset["test"], experiment=experiment, id2label=id2label
)
test_ds_coco_format = CocoDetection(path_output, image_processor, path_anno)

results = run_evaluation(
    test_ds_coco_format=test_ds_coco_format, im_processor=image_processor, model=model
)
...
id2filename_eval = get_filenames_by_ids(
    image_ids=eval_image_ids, annotations=annotations
)
