...
class SaveTrainingMetrics(Callback):
    ...
    def on_train_loader_end(self, context: PhaseContext) -> None:
        experiment.log(name='training_loss_cls', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_cls']))
        experiment.log(name='training_loss_iou', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_iou']))
        experiment.log(name='training_loss_dfl', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_dfl']))
        experiment.log(name='training_loss', type=LogType.LINE, data=float(context.metrics_dict['PPYoloELoss/loss']))

    @multi_process_safe
    def on_validation_loader_end(self, context: PhaseContext) -> None:
        experiment.log(name='validation_loss_cls', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_cls']))
        experiment.log(name='validation_loss_iou', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_iou']))
        experiment.log(name='validation_loss_dfl', type=LogType.LINE,
                       data=float(context.metrics_dict['PPYoloELoss/loss_dfl']))
        experiment.log(name='validation_loss', type=LogType.LINE, data=float(context.metrics_dict['PPYoloELoss/loss']))


experiment = yolonas_utils.get_experiment()
cwd = os.getcwd()
path_dict = {} 
path_dict, labelmap, label_names = yolonas_utils.get_yolo_dataset(experiment,cwd) # get the data 

# trainer params
EXPERIMENT_NAME = experiment.name
CHECKPOINT_DIR = os.path.join(cwd, experiment.name, 'checkpoints')  # path to save the checkpoints to

# dataset params
DATA_DIR = os.path.join(cwd, experiment.base_dir)
TRAIN_IMAGES_DIR = path_dict['train']['dataset_dir']  # child dir of DATA_DIR where train images are
VAL_IMAGES_DIR = path_dict['val']['dataset_dir']  # child dir of DATA_DIR where validation images are
TEST_IMAGES_DIR = path_dict['test']['dataset_dir']  # child dir of DATA_DIR where test images are

TRAIN_LABELS_DIR = os.path.join(cwd, path_dict['train'][
    'yolo_annotation_path'])  # child dir of DATA_DIR where train labels are
VAL_LABELS_DIR = os.path.join(cwd, path_dict['val'][
    'yolo_annotation_path'])  # child dir of DATA_DIR where validation labels are
TEST_LABELS_DIR = os.path.join(cwd,
                               path_dict['test']['yolo_annotation_path'])  # child dir of DATA_DIR where test labels are
...
artifact_namepath = experiment.get_artifact(name='checkpoints_best').filename  # path to the pretrained weights
...
models.convert_to_onnx(model=best_model, input_shape=(3,640,640), out_path=onnx_model_path)
...
test_results = trainer.test(model=best_model,
...
test_results = yolonas_utils.convert_test_results(test_results)
...
for asset in test_image_list:
    bbox_list = yolonas_utils.get_asset_predictions(experiment, best_model, asset, conf=0.1)
