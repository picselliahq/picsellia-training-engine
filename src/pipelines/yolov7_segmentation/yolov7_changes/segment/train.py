...
from utils.loggers import GenericLogger, Loggers
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    data_dict = None
    if RANK in {-1, 0}:
        logger = GenericLogger(opt=opt, console_logger=LOGGER, api_token=opt.api_token, organization_id=opt.organization_id, host=opt.host, experiment_id=opt.experiment_id)
        #loggers = Loggers(save_dir, weights, opt, hyp, LOGGER, api_token=opt.api_token, organization_id=opt.organization_id, host=opt.host, experiment_id=opt.experiment_id)  # loggers instance
        # if loggers.clearml:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--organization_id', type=str, help='Picsellia organization id')
    parser.add_argument('--host', type=str, help='Picsellia host')
    parser.add_argument('--experiment_id', type=str, help='Picsellia experiment id')
