    ...
    def get_dataset_collection(self) -> DatasetCollection:
        ...
        if datasets_count == 3:
            try:
                ...
                raise ResourceNotFoundError(
                    f"Found {datasets_count} attached datasets but {DatasetSplitName.VAL.value} not found"
                )
            try:
                ...
                raise ResourceNotFoundError(
                    f"Found {datasets_count} attached datasets but {DatasetSplitName.TEST.value} not found"
                )
