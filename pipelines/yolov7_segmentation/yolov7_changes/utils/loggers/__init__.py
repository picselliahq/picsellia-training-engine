...
class Loggers:
    ...
    def __init__(
        ...
        if picsellia:
            picsellia_client = Client(
                api_token=api_token, organization_id=organization_id, host=host
            )
            self.picsellia_experiment = picsellia_client.get_experiment_by_id(
                experiment_id
            )
        ...
        if self.wandb or self.clearml:
            ...
            for f in files:
                self.picsellia_experiment.log(
                    name=f.stem, data=str(f), type=LogType.IMAGE
                )
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi):
        ...
        if self.csv:
            ...
            for v in vals:
                try:
                    numeric_vals.append(
                        float(v)
                    )  # Convertir chaque valeur en float si possible
                except ValueError:
                    print(f"Warning: Non-numeric value encountered in `vals`: {v}")
                    numeric_vals.append(
                        0.0
                    )  # Remplacez les valeurs non-numeriques par 0.0 ou une autre valeur par défaut
            ...
            with open(file, "a") as f:
                f.write(
                    s
                    + ("%20.5g," * n % tuple([epoch] + numeric_vals)).rstrip(",")
                    + "\n"
                )
        ...
        for k, v in x.items():
            data = (
                float(v) if not isinstance(v, torch.Tensor) else float(v.cpu().numpy())
            )
    ...
    def on_train_end(self, last, best, epoch, results):
        ...
        for f in files:
            ...
            self.picsellia_experiment.store(
                name="best-model", path=str(best if best.exists() else last)
            )
