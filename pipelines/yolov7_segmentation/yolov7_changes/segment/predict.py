    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                im_masks = plot_masks(
                    im[i], masks, mcolors
                )  # image with masks shape(imh,imw,3)
                annotator.im = scale_masks(
                    im.shape[2:], im_masks, im0.shape
                )  # scale to original h, w
                # Mask plotting ----------------------------------------------------------------------------------------
                if isinstance(masks, torch.Tensor):
                    masks = masks.cpu().numpy()

                # Conversion du masque en une image binaire (0 pour le fond, 255 pour l'objet)
                scaled_masks = np.array(
                    [cv2.resize(mask, (im0.shape[1], im0.shape[0])) for mask in masks]
                )

                # Sauvegarde du masque en niveaux de gris pour chaque objet détecté
                for j, mask in enumerate(scaled_masks):
                    binary_mask = (mask * 255).astype(
                        np.uint8
                    )  # Convertir en uint8 pour OpenCV
                    cv2.imwrite(
                        str(save_dir / "masks" / f"{p.stem}_{j}.png"), binary_mask
                    )
