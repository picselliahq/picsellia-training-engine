...
def get_train_test_val_datasets_from_experiment(
    ...
    is_split = _is_train_test_val_dataset(experiment)
    if is_split:
        print("We found 3 datasets:")
        train_dataset: DatasetVersion = experiment.get_dataset("train")
        print(f"{train_dataset.name}/{train_dataset.version} for training")
        test_dataset: DatasetVersion = experiment.get_dataset("test")
        print(f"{test_dataset.name}/{test_dataset.version} for testing/evaluation")
        val_dataset: DatasetVersion = experiment.get_dataset("val")
        print(f"{val_dataset.name}/{val_dataset.version} for validation")
    else:
        print(
            "We only found one dataset inside your experiment, the train/test/val splits will be "
            "performed automatically."
        )
        train_dataset: DatasetVersion = experiment.list_attached_dataset_versions()[0]
        test_dataset = None
        val_dataset = None
    return is_split, train_dataset, test_dataset, val_dataset


def _is_train_test_val_dataset(experiment: Experiment) -> bool:
    datasets: List[DatasetVersion] = experiment.list_attached_dataset_versions()
    if len(datasets) < 3:
        return False

    template = ["train", "test", "val"]
    ok_counter = 0
